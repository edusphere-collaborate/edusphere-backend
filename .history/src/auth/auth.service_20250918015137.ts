...
export class AuthService {
  ...
  async register(registerDto: RegisterUserDto) {
    ...
    const token = await this.generateToken({
      id: user.id,
      email: user.email as string, // Add type assertion as we've verified email exists
      role: user.role,
    });
  ...
  async validateUser(loginDto: UserLoginDto) {
    ...
    const token = await this.generateToken({
      id: user.id,
      email: user.email as string, // Add type assertion
      role: user.role,
    });
    return { ...user, token };
  }
  ...
  async verifyResetToken(rawToken: string) {
    ...
    if (!email) {
      return { success: true, valid: false }; // Fail safely if email is null
    }
  ...
  async changePassword(
    ...
    if (!user) throw new NotFoundException('User not found');
    if (!user.password) throw new ForbiddenException('Cannot change password for OAuth users');
    ...
    if (user.email) { // Only send email if email exists
      const d = new Date();
      await this.mailService.sendMail(
        user.email,
        'Your EduSphere password was changed',
    ...
    if (!user) {
      user = await this.prismaService.user.create({
        data: {
          ...
          // Use the correct field name from your Prisma schema for storing profile pictures
          // This might be 'avatar', 'imageUrl', 'profilePicture', etc.
          // If unsure, check your Prisma schema file
          picture: profile.picture, // Changed field name to match your schema
