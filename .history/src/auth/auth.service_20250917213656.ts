...
export class AuthService {
  ...
  async forgotPassword(email: string) {
    ...
    await this.mailService.sendMail(
      ...
      this.mailService.resetPasswordEmailHtml(user.firstName ?? '', rawToken, resetLink, 1),
    );

    return { success: true, message: 'If that email exists, a reset link will be sent.', expiresIn: Math.floor((expiresAt.getTime() - Date.now()) / 1000) };
  }
}
