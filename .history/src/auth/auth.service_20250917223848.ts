...
export class AuthService {
  ...
  async resetPassword(token: string, newPassword: string, confirmPassword: string) {
    ...
    const tokenRecord = await this.tokenService.validateAndConsumeToken(token, TokenType.PASSWORD_RESET);
    if (!tokenRecord) {
      return { success: false, message: 'Invalid or expired token' };
    }
    ...
    await this.mailService.sendMail(
      ...
      this.mailService.passwordChangedNotificationHtml(user.firstName ?? '', d.toLocaleDateString(), d.toLocaleTimeString()),
    );

    return {
      success: true,
      message: 'Password reset successfully',
      user: { id: user.id, email: user.email, passwordChangedAt: new Date().toISOString() },
    };
  }
}
