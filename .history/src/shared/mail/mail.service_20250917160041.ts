...
export class MailService {
  ...
  constructor(private config: ConfigService) {
    this.fromEmail = this.config.get('FROM_EMAIL') || 'no-reply@example.com';
  ...
  verificationEmailHtml(firstName: string, verificationCode: string, verificationLink: string, expiresInHours = 24) {
    return `
  ...
  resetPasswordEmailHtml(firstName: string, resetCode: string, resetLink: string, expiresInHours = 1) {
    return `
      <p>Hello ${firstName ?? 'User'},</p>
  ...
  passwordChangedNotificationHtml(firstName: string, changeDate: string, changeTime: string) {
    return `
      <p>Hello ${firstName ?? 'User'},</p>
